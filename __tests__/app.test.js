...
describe('goal routes', () => {
  ...
  it('GET all goals', async () => {
    ...
    return request(app)
      .get('/api/v1/goals')
      .then((res) => {
        expect(res.body).toEqual([{ ...goal1, discovery: expect.any(String) }, { ...goal2, discovery: expect.any(String) }, { ...goal3, discovery: expect.any(String) }]);
      });
  });
  ...
  it('PUT updates a single goal', async () => {
    ...
    const res = await request(app)
      ...
      .send({ achieved: true });
    expect(res.body).toEqual({ id:'1', ...goal2, achieved: true, discovery: expect.any(String) });
  });
